...
const NotificationItem: React.FC<{
  ...
  return (
    ...
    >
      ...
      {isUnread && (
        <span className="flex-shrink-0 mt-2 h-2 w-2 rounded-full bg-[#2B70B2]" />
      )}
    </button>
  );
};
      ...
      <SheetContent side="right" className="w-full sm:max-w-sm p-0 flex flex-col">
        <SheetHeader className="px-5 py-4 border-b border-gray-100 flex-shrink-0">
          <div className="flex items-center justify-between">
            <SheetTitle className="text-base font-semibold text-gray-900 flex items-center gap-2">
              <Bell className="h-4 w-4 text-[#2B70B2]" />
              Notificações
              {unreadCount > 0 && (
            ...
            {unreadCount > 0 && (
              <button
                onClick={markAllRead}
                className="flex items-center gap-1 text-xs text-[#2B70B2] hover:underline font-medium"
              >
        ...
        <div className="flex-1 overflow-y-auto">
          {isLoading ? (
            <div className="py-16 text-center">
              <div className="animate-spin rounded-full h-6 w-6 border-2 border-[#2B70B2] border-t-transparent mx-auto" />
            ...
            <div className="py-16 text-center px-4">
              ...
              <p className="text-xs text-gray-400 mt-1">
                Atualizações dos seus agendamentos aparecerão aqui.
              </p>
            </div>
          ) : (
            <div>
        ...
        <div className="flex-shrink-0 border-t border-gray-100 bg-gray-50 px-5 py-3">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 text-sm font-medium text-[#2B70B2] hover:underline py-1"
          >
